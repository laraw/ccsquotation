...
export class CentreQuoteComponent implements OnInit {
  ...
  ngOnInit(): void { this.ccsEntered = false; this.sessionsEntered = false; showWeek2: false; this.ccsCalculateRequired = false; }
...
setEntitlementOption(event:any) {
    console.log(event.target.value);
    if(event.target.value === "option1") {
        this.ccsCalculateRequired = true;
    }
    else {
        this.ccsCalculateRequired = false;
    }
}
  ...
  calculateCCS() {
    ...
    let ccshourlyRateCap = 12.20;

    if (isSchoolAged) {
    
      ccshourlyRateCap = 10.67
    }
    //this.week1fee = 1000;
